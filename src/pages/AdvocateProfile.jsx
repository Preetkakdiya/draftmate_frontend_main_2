  ...
  useEffect(() => {
    ...
    const fetchProfile = async () => {
      try {
        ...
        // 1. Try fetching from public backend API
        try {
          ...
          console.info('Public API fetch note:', apiErr);
        }

        // 2. If API didn't return profile, check local storage
        if (!foundProfile) {
        ...
        if (foundProfile) {
          if (typeof foundProfile.languages === 'string') {
            try { foundProfile.languages = JSON.parse(foundProfile.languages); }
            catch { foundProfile.languages = foundProfile.languages.split(',').map(l => l.trim()).filter(Boolean); }
          }
      ...
      <div className="relative bg-[#0B1B33] overflow-hidden">
        ...
        <div className="relative z-10 max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8 py-12 md:py-20 flex flex-col md:flex-row items-center gap-8 justify-between">
          ...
          <div className="flex-shrink-0 relative group">
            <div className="w-32 h-32 md:w-48 md:h-48 rounded-full border-4 border-slate-700 shadow-2xl relative overflow-hidden bg-slate-900 group-hover:border-blue-500 transition-colors duration-300">
              {hasValidImage ? (
                <img src={profile.profile_image_url} alt={name} className="w-full h-full object-cover" />
              ) : (
                <div className="w-full h-full flex items-center justify-center text-white text-4xl md:text-6xl font-extrabold shadow-inner" style={{ background: 'linear-gradient(135deg, #2563EB 0%, #1D4ED8 50%, #0F1C2E 100%)' }}>
                  {getInitials(name)}
                </div>
              )}
            </div>
          ...
          <div className="flex-shrink-0 flex flex-col gap-3 w-full md:w-auto">
            <button type="button" onClick={() => setIsConsultModalOpen(true)} className="w-full md:w-64 h-14 text-base font-extrabold text-white bg-blue-600 hover:bg-blue-500 rounded-xl shadow-lg shadow-blue-600/30 hover:scale-[1.02] active:scale-[0.98] transition-all flex items-center justify-center gap-2 cursor-pointer">
              <CalendarCheck className="w-5 h-5" /> Book Consultation
            </button>
            <div className="flex gap-3">
              <button type="button" onClick={() => setIsContactModalOpen(true)} className="flex-1 h-12 text-sm font-bold text-white bg-white/10 hover:bg-white/20 border border-white/25 rounded-xl backdrop-blur-md transition-all flex items-center justify-center gap-2 cursor-pointer">
                <MessageSquare className="w-4 h-4" /> Message
              </button>
              <button type="button" onClick={() => setIsShareModalOpen(true)} className="w-12 h-12 text-white bg-white/10 hover:bg-white/20 border border-white/25 rounded-xl backdrop-blur-md transition-all flex items-center justify-center shrink-0 cursor-pointer" aria-label="Share">
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          <div className="w-full lg:w-[26%] flex-shrink-0 space-y-6">
                ...
                <div className="bg-white rounded-2xl shadow-sm border border-slate-200/60 p-6">
                  ...
                  <div className="space-y-4">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="font-bold text-slate-900">{casesWon}</span>
                    </div>
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-slate-200/60 p-6 flex flex-col items-center text-center">
              ...
              <button type="button" onClick={() => setIsConsultModalOpen(true)} className="w-full h-12 rounded-xl bg-blue-600 hover:bg-blue-700 text-white font-bold transition-all shadow-md shadow-blue-600/20 flex items-center justify-center gap-2 cursor-pointer">
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-slate-200/60 p-6 flex flex-col items-center text-center">
              ...
              <button type="button" onClick={() => setIsContactModalOpen(true)} className="w-full h-12 rounded-xl bg-slate-900 hover:bg-slate-800 text-white font-bold transition-all flex items-center justify-center gap-2 cursor-pointer">
