  ...
  return (
    <div className="flex h-screen bg-[#F8FAFF] overflow-hidden font-sans">
      ...
      >
          ...
          {SIDEBAR_ITEMS.map((item) => {
            ...
            return (
              <Link key={item.label} to={item.path} className="block">
                <motion.div
                  whileHover={{ x: 4 }}
                  className={`flex items-center gap-3 px-3 py-2.5 rounded-xl transition-all duration-200 group relative ${isActive ? "bg-blue-50 text-blue-600 font-bold" : "text-slate-500 hover:bg-blue-50 hover:text-blue-600 font-medium"
                    }`}
                >
        ...
        {/* Footer Utilities with Collapse Toggle */}
        <div className="px-3 py-2 border-t border-slate-100">
          <button
            onClick={() => setIsUtilitiesExpanded(!isUtilitiesExpanded)}
            className="w-full flex items-center justify-between px-3 py-2 text-[11px] font-bold text-slate-400 hover:text-slate-600 hover:bg-slate-50 rounded-lg transition-colors uppercase tracking-widest"
          >
            {isSidebarOpen ? <span>Account Center</span> : <Settings className="w-4 h-4 mx-auto" />}
            {isSidebarOpen && (
              <ChevronRight className={`w-3.5 h-3.5 transition-transform duration-300 ${isUtilitiesExpanded ? 'rotate-90 text-blue-500' : ''}`} />
            )}
          </button>
      ...
      <div className="flex-1 flex flex-col h-screen overflow-hidden relative">
        {/* ── TOP NAVBAR ── */}
        {/* 🔥 FIX: Added logic to hide the entire header on desktop if not on dashboard */}
        <header className={`absolute top-0 inset-x-0 z-30 transition-all duration-300 px-4 md:px-8 py-4 ${scrolled ? 'transform -translate-y-full' : 'translate-y-0'} ${!isDashboardRoute ? 'lg:hidden' : ''}`}>
          <div className="bg-white/80 backdrop-blur-xl border border-white/40 shadow-[0_8px_30px_rgb(0,0,0,0.04)] rounded-full h-14 flex items-center justify-between px-4 pl-6 w-full">

            {/* Mobile Menu Toggle - Kept OUTSIDE the condition */}
            <button onClick={() => setMobileMenuOpen(true)} className="lg:hidden p-2 text-slate-500 hover:text-[#0F1C2E] mr-4">
            ...
            {isDashboardRoute && (
              <div className="flex items-center justify-between flex-1 w-full animate-fade-in-up">
                ...
                {/* 2. Right Side Icons Div (Credits, Top-up, Bell) */}
