...
const ExplainDrawer = ({ isOpen, onClose, act, chapter, section, content, type }) => {
  ...
  useEffect(() => {
    if (isOpen) {
      if (type === 'judgment' && content) {
        // For judgments, we'll use mock data directly since we don't have an AI service yet
        setExplanation({
          caseFacts: content.summary,
          legalIssues: content.legalIssue,
          courtReasoning: content.ratiodecidendi,
          finalDecision: `The ${content.court} delivered this judgment in ${content.year}.`,
          practicalImpact: content.keyPrinciples.join(' '),
          model: 'DraftMate AI v1.0'
        });
      } else if (act && section) {
        fetchExplanation();
      }
